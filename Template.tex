
%%%%%%%% frame
\begin{frame}\frametitle{}

\end{frame}

%%%%%%%% frame with itemize
\begin{frame}\frametitle{}
  \begin{itemize}
    \item 
  \end{itemize}
\end{frame}

%%%%%%%%    eqnarray
\begin{eqnarray*}
  
\end{eqnarray*}

%%%%%%%%    eqnarray
\begin{eqnarray}
  
  \label{}
\end{eqnarray}

%%%%%%%%    equation
\begin{equation}
  
  \label{}
\end{equation}

%%%%%%%%    equation
\begin{equation*}
  
\end{equation*}

%%%%%%%%
\[
  
\]

%%%%%%%%
$$  $$

%%%%%%%%    align
\begin{align}
  
  \label{}
\end{align}

%%%%%%%%
\begin{align*}
  
\end{align*}

\begin{frame}
  \begin{block}{Test}
    
  \end{block}
\end{frame}

\begin{columns}[t]
  \column{.5\textwidth}

  \column{.5\textwidth}
\end{columns}

\begin{frame}\frametitle{LHC Layouts}\fontsize{9pt}{7.5}\selectfont
        \begin{columns}[T]
        \column{.5\textwidth}
                \begin{block}{}
                        \begin{center}

                        \end{center}
                \end{block}

        \column{.45\textwidth}   
                \begin{block}{}
                \begin{itemize}
                  \item
                \end{itemize}
                \end{block}
        \end{columns}
\end{frame}


\begin{figure}[<+htpb+>]
  \centering
  \includegraphics{<+file+>}
  \caption{<+caption text+>}
  \label{fig:<+label+>}
\end{figure}

\begin{tabular}[<+position+>]{<+format+>}
  <++>
\end{tabular}<++>

%%%		Put figure one above other to point something in next click
\begin{frame}\frametitle{This is your title place }
	 \begin{tikzpicture}[every node/.style={anchor=center}]
 		\node(a) at (8,6){\includegraphics[width=12cm,height=7cm]{figure.pdf}};
		\pause
		\node(b) at (10,7){\includegraphics[width=6cm]{figure_zoomed.pdf}};
		\draw[red,thick,->](8,8)--(3.5,6.5);
	 \end{tikzpicture}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	WRITE SOMETHING ON FIGURE
\begin{tikzpicture}
\draw (0, 0) node[inner sep=0] 
{\includegraphics[width=12cm,height=9cm]{something.pdf}};
\draw (2, 3) node {\color{red} Comment On Plot};
\end{tikzpicture}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[shrink]\frametitle{Put the title name}
{\scriptsize \begin{tabular}{|p{1.5cm}|p{1cm}|c|c|c|c|c|}
    	\hline
	Sample & Total Event  & muon $p_T, \eta$ & pfMET & $Jet_{|\eta|}$ &  $Jet_{p_T}$  & $dR>0.5$\\
	\hline
	\hline
	signal & 8 & 8 & 7 & 7  & 7 & 7\\
	\hline
	Data/mc & 0.6210 & 0.7379 & 0.7263 & 0.8272 & 0.8512 & 0.8512\\
	\hline
\end{tabular}
}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{frame}[c]
	\begin{center}
	\Huge Thanks
	\end{center}
\end{frame}

\begin{frame}[c]
	\begin{center}
	\Huge Backup Slides
	\end{center}
\end{frame}



